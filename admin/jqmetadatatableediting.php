...
$crudColumns =  array(
	...
	//ccmeta_id 	doc_level 	doc_type 	doc_order 	doc_col_caption 	doc_col_desc 	doc_col_tooltip 	doc_col_type 	is_multiple
	'id'=>'ccmeta_id'//the first column should be named as 'id'
	//,'protocol'=>'doc_level'
	//,'method'=>'doc_role'
	//,'url'=>'doc_order'
	//,'site'=>'doc_col_caption'
	//,'media_type'=>'doc_col_desc'
	//'ccmeta_id'=>'ccmeta_id'
	,'doc_level'=>'doc_level'
	,'doc_type'=>'doc_type'
	,'doc_order'=>'doc_order'
	,'doc_col_caption'=>'doc_col_caption'
	,'doc_col_desc'=>'doc_col_desc'
	,'doc_col_tooltip'=>'doc_col_tooltip'
	,'doc_col_type'=>'doc_col_type'
	,'is_multiple'=>'is_multiple'
);
$crudTableName = 'cc_meta';
$postConfig['id'] = 'ccmeta_id'; 
...
switch($postConfig['action']){
	...
	case $crudConfig['update']:
		...
		$sql .= ' where ccmeta_id = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where ccmeta_id = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
